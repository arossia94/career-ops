...
- **`score`** (0–100) is the reference model's fit verdict. It is profile-relative by
...
where a cheaper model is most likely to diverge, and scores span 64–86 so the
...
```json
{
  ...
  "label": { "archetype": "AI Platform / LLMOps", "score": 84, "provenance": "reference-frozen-v1" }
}
```
