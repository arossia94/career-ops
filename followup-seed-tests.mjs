...
// ── Test 1: round-trip — pin date derives from notes, not the tracker date column ──
{
  ...
  writeTracker(sb, [trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-06-20. Great team.')]);
...
{
  const sb = makeSandbox();
  writeTracker(sb, [trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-06-20.')]);
  run(['1'], sb);
  const content = readFileSync(sb.followups, 'utf-8');
...
// ── Test 3: idempotency + --force appends a second pin (last wins) ─────────
{
  ...
  writeTracker(sb2, [trackerRow(2, '2026-05-01', 'Globex', 'Manager', '80/100', 'Applied', 'Applied 2026-06-01.')]);
  const tableHeader = [
    '# Follow-ups',
    '',
...
// ── Test 4: date resolution order — --date beats notes; notes beat today; column never used ──
{
  ...
  writeTracker(sb, [trackerRow(1, '2026-01-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-06-20.')]);
...
// ── Test 5: missing file gets exact canonical header; append preserves bytes ──
{
  ...
  writeTracker(sb, [
    trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-06-20.'),
    trackerRow(2, '2026-05-01', 'Globex', 'Manager', '80/100', 'Applied', 'Applied 2026-06-15.'),
  ]);
  run(['2'], sb);
...
{
  const sb = makeSandbox();
  writeTracker(sb, [trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Evaluated', 'no applied date yet')]);
  const res = run(['1'], sb);
...
// ── Test 8: --backfill seeds only unpinned Applied rows; re-run seeds 0 ────
{
  const sb = makeSandbox();
  writeTracker(sb, [
    trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-06-01.'),   // Applied, unpinned
    trackerRow(2, '2026-05-01', 'Globex', 'Manager', '80/100', 'Applied', 'Applied 2026-06-05.'),  // Applied, will be pre-pinned
    trackerRow(3, '2026-05-01', 'Initech', 'Analyst', '60/100', 'Evaluated', 'not applied'),        // Evaluated
    trackerRow(4, '2026-05-01', 'Umbrella', 'Scientist', '40/100', 'Rejected', 'Applied 2026-04-01. rejected'), // Rejected
  ]);
...
// ── Test 10: stale-pin harmlessness — pin still parses after status flips to Rejected ──
{
  ...
  writeTracker(sb, [trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Rejected', 'Applied 2026-06-20. rejected after onsite')]);
  const content = readFileSync(sb.followups, 'utf-8');
  const overrides = parseNextOverrides(content);
...
{
  const sb = makeSandbox();
  writeTracker(sb, [trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-02-31. Bad date.')]);
  const res = run(['1', '--json'], sb);
...
// ── Test 12: backfill skips (not aborts on) a row with an impossible notes date ──
{
  const sb = makeSandbox();
  writeTracker(sb, [
    trackerRow(1, '2026-05-01', 'Acme', 'Engineer', '80/100', 'Applied', 'Applied 2026-02-31. Bad date.'),
    trackerRow(2, '2026-05-02', 'Globex', 'Engineer', '84/100', 'Applied', 'Applied 2026-06-20.'),
  ]);
  const res = run(['--backfill', '--json'], sb);
