...
async function cmdEvaluate(input, ctx) {
  ...
  try {
    ...
    const scoreStr    = isFinite(scoreValue) ? `${Math.round(scoreValue)}/100` : '';
...
async function cmdApply(ref, ctx) {
  ...
  const scoreMatch = reportContent.match(/^\s*\*?\*?\s*(?:score|puntuaci[oó]n)\s*:\s*\*?\*?\s*(\d+(?:\.\d+)?)\s*\/\s*100/im);
  const scoreValue = scoreMatch ? parseFloat(scoreMatch[1]) : NaN;
  if (isFinite(scoreValue) && scoreValue < 70) {
    console.log(`\n⚠️  This report scored ${Math.round(scoreValue)}/100 — below the 70/100 threshold.`);
