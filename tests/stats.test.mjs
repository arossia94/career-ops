...
try {
  ...
  const trackerMd = [
    ...
    '|---|------|---------|------|-------|--------|-----|--------|-------|',
    '| 1 | 2026-06-01 | Acme | Eng | 90/100 | Applied | ✅ | [1](../reports/001-acme-2026-06-01.md) | note |',
    '| 2 | 2026-06-02 | Beta | Eng | 76/100 | Evaluated | ❌ | [2](../reports/002-beta-2026-06-02.md) | note |',
    '| 3 | 2026-06-03 | Gama | Eng | 84/100 | Interview | ✅ | ❌ | note |',
  ].join('\r\n');
  const t = stats.computeTrackerStats(trackerMd);
  if (t.total === 3 && t.byStatus.Applied === 1 && t.byStatus.Evaluated === 1
      && t.byStatus.Interview === 1 && t.avgScore === 83.3 && t.avgScoreApplied === 87
      && t.topScore === 90 && t.pdfPct === 66.7 && t.reportPct === 66.7 && t.activeApps === 2) {
  ...
  const hiredMd = [
    '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |',
    '|---|------|---------|------|-------|--------|-----|--------|-------|',
    '| 1 | 2026-06-01 | Acme | Eng | 90/100 | Hired | ✅ | ❌ | landed |',
    '| 2 | 2026-06-02 | Beta | Eng | 70/100 | Applied | ✅ | ❌ | sent |',
  ].join('\n');
  const th = stats.computeTrackerStats(hiredMd);
  if (th.byStatus.Hired === 1 && th.byStatus.Unknown === undefined && th.avgScoreApplied === 80) {
  ...
  const coldTrackerMd = [
    '| # | Date | Company | Role | Score | Status | PDF | Report | Notes |',
    '|---|------|---------|------|-------|--------|-----|--------|-------|',
    '| 1 | 2026-05-01 | Acme | Eng | 90/100 | Applied | ✅ | ❌ | note |',
    '| 2 | 2026-05-01 | Beta | Eng | 80/100 | Applied | ✅ | ❌ | note |',
    '| 3 | 2026-05-01 | Gama | Eng | 78/100 | Interview | ✅ | ❌ | note |',
  ].join('\n');
