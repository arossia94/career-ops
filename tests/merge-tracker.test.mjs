...
try {
  ...
  const hired = runMerge({
    '1-acme.tsv': '1\t2026-01-01\tAcme\tML Eng\tHired\t90/100\t✅\t[1](reports/1-acme-2026-01-01.md)\tlanded the job\n',
  });
  ...
  const accepted = runMerge({
    '2-globex.tsv': '2\t2026-01-02\tGlobex\tData Eng\taccepted\t80/100\t✅\t[2](reports/2-globex-2026-01-02.md)\toffer accepted\n',
  });
...
try {
  const seeded =
    '| 1 | 2026-01-01 | Acme | Staff Data Platform Engineer | 80/100 | Evaluated | ❌ | ' +
  ...
  const res = runMergeDetailed({
    ...
    'a-001-acme.tsv': '1\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tsecond look\n',
    'b-001-acme.tsv': '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tthird look\n',
  }, { rows: seeded });

  const rows = dataRows(res.tracker);
  if (rows.length === 1 && /94\/100/.test(rows[0])) {
  ...
  const downgrade = runMergeDetailed({
    'a-001-acme.tsv': '1\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-01-01.md)\thigh\n',
    'b-001-acme.tsv': '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tlow\n',
  }, { rows: seeded });
  ...
  if (downgradeRows.length === 1 && /94\/100/.test(downgradeRows[0]) && /⏭️1 skipped/.test(downgrade.output)) {
...
try {
  const sameRole = runMergeDetailed({
    '010-acme.tsv': '10\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[10](reports/010-acme-2026-02-01.md)\tfirst pass\n',
    '011-acme.tsv': '11\t2026-02-02\tAcme\tStaff Data Platform Engineer\tEvaluated\t92/100\t❌\t[11](reports/011-acme-2026-02-02.md)\tsecond pass\n',
  });
  ...
  if (sameRows.length === 1 && /92\/100/.test(sameRows[0])) {
  ...
  const distinct = runMergeDetailed({
    '020-acme.tsv': '20\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[20](reports/020-acme-2026-02-01.md)\tplatform\n',
    '021-acme.tsv': '21\t2026-02-02\tAcme\tDirector of Product Marketing\tEvaluated\t92/100\t❌\t[21](reports/021-acme-2026-02-02.md)\tmarketing\n',
  });
...
try {
  const APPLIED_ROW =
    '| 1 | 2026-01-01 | Acme | Staff Data Platform Engineer | 80/100 | Applied | ❌ | ' +
  ...
  const upgraded = runMergeDetailed({
    '001-acme.tsv': '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tre-scored after JD refresh\n',
  }, { rows: APPLIED_ROW });
  const upgradedRow = dataRows(upgraded.tracker)[0] || '';

  if (/94\/100/.test(upgradedRow) && /re-scored after JD refresh/.test(upgradedRow) && /Re-eval 2026-03-01/.test(upgradedRow)) {
  ...
  const sibling = runMergeDetailed({
    '002-acme.tsv': '2\t2026-04-01\tAcme\tSenior Staff Data Platform Engineer\tEvaluated\t98/100\t❌\t[2](reports/002-acme-2026-04-01.md)\tReq R_1499999 separate posting\n',
  }, { rows: `${upgradedRow}\n` });
...
try {
  const ADDITION = '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-03-01.md)\tonly evaluation\n';
...
try {
  const twoCompanies = runMergeDetailed({
    '030-zeta.tsv': '30\t2026-02-01\t株式会社ゼータ\tデータエンジニア\tEvaluated\t84/100\t❌\t[30](reports/030-zeta-2026-02-01.md)\tzeta eval\n',
    '031-omega.tsv': '31\t2026-02-02\t合同会社オメガ\tデータエンジニア\tEvaluated\t92/100\t❌\t[31](reports/031-omega-2026-02-02.md)\tomega eval\n',
  });
  ...
  const sameCompany = runMergeDetailed({
    '040-zeta.tsv': '40\t2026-02-01\t株式会社ゼータ\tデータエンジニア\tEvaluated\t84/100\t❌\t[40](reports/040-zeta-2026-02-01.md)\tfirst pass\n',
    '041-zeta.tsv': '41\t2026-02-02\t株式会社ゼータ\tデータエンジニア\tEvaluated\t92/100\t❌\t[41](reports/041-zeta-2026-02-02.md)\tsecond pass\n',
  });
  const sameCompanyRows = dataRows(sameCompany.tracker);
  if (sameCompanyRows.length === 1 && /92\/100/.test(sameCompanyRows[0])) {
...
try {
  ...
  const substringNote = runMergeDetailed({
    '001-acme.tsv': '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tRemote\n',
  }, { rows: REMOTE_ROW });
  ...
  const repeatNote = runMergeDetailed({
    '001-acme.tsv': '1\t2026-03-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t94/100\t❌\t[1](reports/001-acme-2026-01-01.md)\tRemote OK\n',
  }, { rows: REMOTE_ROW });
...
try {
  const collision = runMergeDetailed({
    '050-acme-a.tsv': '5\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[5](reports/005-acme-2026-02-01.md)\tplatform\n',
    '051-acme-b.tsv': '5\t2026-02-02\tAcme\tDirector of Product Marketing\tEvaluated\t92/100\t❌\t[6](reports/006-acme-2026-02-02.md)\tmarketing\n',
  });
  ...
  const sameRoleCollision = runMergeDetailed({
    '060-acme-a.tsv': '7\t2026-02-01\tAcme\tStaff Data Platform Engineer\tEvaluated\t84/100\t❌\t[7](reports/007-acme-2026-02-01.md)\tfirst pass\n',
    '061-acme-b.tsv': '7\t2026-02-02\tAcme\tStaff Data Platform Engineer\tEvaluated\t92/100\t❌\t[8](reports/008-acme-2026-02-02.md)\tsecond pass\n',
  });
  const sameRoleRows = dataRows(sameRoleCollision.tracker);
  if (sameRoleRows.length === 1 && /92\/100/.test(sameRoleRows[0])) {
