...
The evaluation uses 6 blocks (A-F) with a global score of 0-100:
...
3. **If most `require` criteria have positive evidence** → score 80-100.
4. **If some criteria have positive evidence, and none are contradicted** → score 60.
5. **If evidence contradicts the `require` criteria** → **cap this dimension at 40/100**, and add an explicit line to Block A's Culture Screen field (see `oferta.md`) naming what's missing or contradicted. Do not let a strong CV-match score silently compensate for this — surface it, don't bury it.
6. **If no evidence exists for any `require` criterion** → score 60 by default, unless `culture_screen.deprioritize_if_absent: true` is set, in which case **cap this dimension at 40/100**.
7. A role scoring 90+ overall but 40 or below on Cultural signals must carry an explicit warning in the report: "High technical fit, unconfirmed/poor culture fit — verify before applying."
