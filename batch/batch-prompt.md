...
- Block caps, such as "cap Block A at 60/100 if title contains Lead/Head/Principal"
...
|-----------|-------|
| CV match | XX/100 |
| North Star alignment | XX/100 |
| Compensation | XX/100 |
| Culture / working model | XX/100 |
| Red flags | -X if any |
| **Global** | **XX/100** |
...
- `score` is numeric only, without `/100`.
...
```text
{{REPORT_NUM}}\t{{DATE}}\t{company}\t{role}\t{status}\t{score}/100\t{pdf_emoji}\t[{{REPORT_NUM}}](reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md)\t{one_sentence_note}
```
