 ...
 *
 * Weighting: each report contributes (100 − score) per skill it names — a
 * 42/100 report says more about your gaps than a 90/100 one. A skill is counted
 ...
 * (score < 70) reports naming the gap — NOT quantiles, which are noise at
...
export function parseReportGaps(content) {
  ...
  if (score === null) {
    const glMatch = plain.match(/\|\s*(?:Global)\s*\|\s*([\d.]+)\/100\s*\|/i);
...
export function aggregateGaps(reports, knownSkills) {
  ...
  for (const report of reports) {
    ...
    for (const skill of skills) {
      ...
      const weight = Number.isFinite(report.score) ? Math.max(0, 100 - report.score) : 1.0;
...
function runSelfTest() {
  ...
  const { gaps: g1 } = aggregateGaps(
    [{ num: 1, score: 40, gapText: 'Missing JavaScript and GCP experience' }],
    cvSkills
  );
  ...
  const { gaps: g2, excludedAsKnown: ex2 } = aggregateGaps(
    [{ num: 2, score: 60, gapText: 'Needs Java and Kubernetes' }],
  ...
  const { gaps: g3 } = aggregateGaps(
    [
      { num: 3, score: 40, gapText: 'Kubernetes Kubernetes Kubernetes' },
      { num: 4, score: 90, gapText: 'Kubernetes' },
    ],
    new Set()
  );
  ...
  if (!k) failures.push('Kubernetes not aggregated');
    ...
    if (Math.abs(k.weightedScore - 70) > 1e-9) failures.push(`weightedScore expected 70, got ${k.weightedScore}`);
  }

  // Tiering: 3/5 low-fit reports naming a skill → Critical; 1/5 → Low
  const lowFitReports = [
    { num: 10, score: 40, gapText: 'Terraform' },
    { num: 11, score: 50, gapText: 'Terraform' },
    { num: 12, score: 60, gapText: 'Terraform and Spark' },
    { num: 13, score: 65, gapText: 'nothing here' },
    { num: 14, score: 68, gapText: 'nothing here' },
  ];
...
\`\`\`
`);
  if (parsed.score !== 64) failures.push(`report score expected 64, got ${parsed.score}`);
