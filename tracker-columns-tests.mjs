...
|---|------|---------|------|----------|-------|--------|-----|--------|-------|
| 1 | 2026-01-01 | Acme | Engineer | Remote | 80/100 | Applied | ✅ | — | seed row |
`;
...
|---|------|---------|------|-------|--------|-----|--------|-------|
| 1 | 2026-01-01 | Acme | Engineer | 80/100 | Applied | ✅ | — | seed row |
`;
...
// Score into Status and folded the real Notes cell away.
{
  ...
  if (sync.code === 0 && query.code === 0 && row) {
    ...
    if (row.score === '80/100') pass('tracker.mjs: Score not shifted on 10-col tracker');
...
// keys like "engineer::remote" and scan dedup missed real matches.
{
  ...
  if (![...seen].some(k => k.includes('remote') || k.includes('80/100'))) {
...
| 2 | 2026-01-03 | Globex | Manager | Berlin | 90/100 | DUPLICADO de #1 | ❌ | — | keep me |
`;
  ...
  if (globex[7] === 'Discarded' && globex[6] === '90/100'
...
|---|------|---------|----------|------|-------|--------|-----|--------|-------|
| 1 | 2026-01-01 | Acme | high | Engineer | 80/100 | Applied | ✅ | — | seed row |
`;
  ...
  if (sync.code === 0 && row && row.role === 'Engineer' && row.score === '80/100' && row.status === 'Applied') {
...
|---|------|---------|------|----------|-------|--------|-----|--------|----------|-------|
| 1 | 2026-01-01 | Acme | Engineer | Remote | 80/100 | Applied | ✅ | — | high | seed row |
`;
  ...
  if (r && r.score === '80/100' && r.status === 'Applied') {
...
|---|------|---------|-----|------|-------|--------|-----|--------|-------|
| 1 | 2026-01-01 | Acme | — | Engineer | 80/100 | Applied | ✅ | — | direct seed row |
| 2 | 2026-01-05 | ? | Hays | Data Engineer | 84/100 | Applied | ✅ | — | fintech, Leeds |
`;
...
// ── Test 9: parseTrackerRow surfaces the Via column ─────────────────────────
{
  ...
  if (direct && direct.via === '—' && direct.role === 'Engineer' && direct.score === '80/100') {
...
{
  const TSV_VIA = '3\t2026-02-02\t?\tPlatform Engineer\tApplied\t82/100\t✅\t—\tblind agency listing\tvia=Hays\n';
  ...
  if (res.code === 0 && cells[3] === '?' && cells[4] === 'Hays' && cells[6] === '82/100' && cells[7] === 'Applied') {
...
{
  const OTHER_AGENCY = '6\t2026-02-02\t?\tData Engineer\tApplied\t90/100\t✅\t—\tsame role, other agency\tvia=Randstad\n';
  const SAME_AGENCY = '7\t2026-02-03\t?\tData Engineer\tApplied\t92/100\t✅\t—\tre-blast, higher score\tvia=Hays\n';
  ...
  if (res2.code === 0 && hays.length === 1 && hays[0].includes('92/100')) {
...
{
  const FIRST = '2\t2026-02-02\t?\tData Engineer\tApplied\t82/100\t✅\t—\tblind listing\tvia=Hays\n';
  const REBLAST = '3\t2026-02-10\t?\tData Engineer\tApplied\t86/100\t✅\t—\tre-blast, higher score\tvia=Hays\n';
  ...
  if (res1.code === 0 && res2.code === 0 && blind.length === 1 && blind[0].includes('86/100') && /1 updated/.test(res2.stdout)) {
...
// ── Test 13: --migrate-via inserts the column, idempotently ─────────────────
{
  ...
  if (first.code === 0 && headCells[4] === 'Via' && headCells[3] === 'Company' && seedCells[4] === '—' && seedCells[6] === '80/100') {
...
| # | Date | Company | Via | Role | Score | Status | PDF | Report | Notes |
|---|------|---------|-----|------|-------|--------|-----|--------|-------|
| 1 | 2026-01-05 | ? | Hays | Data Engineer | 84/100 | Evaluated | ✅ | — | fintech, Leeds |
| 2 | 2026-01-20 | ? | Hays | Data Engineer | 86/100 | Evaluated | ✅ | — | re-blast of same listing |
| 3 | 2026-01-06 | ? | Randstad | Data Engineer | 80/100 | Evaluated | ✅ | — | different channel |
| 4 | 2026-01-10 | ? | Hays | Platform Engineer | 78/100 | Evaluated | ✅ | — | old listing |
| 5 | 2026-06-01 | ? | Hays | Platform Engineer | 88/100 | Evaluated | ✅ | — | far outside window |
`;
  ...
  if (res.code === 0 && dataEng.length === 2 && dataEng.some(l => l.includes('Randstad')) && dataEng.some(l => l.includes('86/100'))) {
...
| # | Date | Company | Via | Role | Score | Status | PDF | Report | Notes |
|---|------|---------|-----|------|-------|--------|-----|--------|-------|
| 1 | 2026-01-05 | ? | — | Data Engineer | 84/100 | Evaluated | ✅ | — | blind row, no agency |
| 2 | 2026-01-06 | Confidential | Hays | ML Engineer | 80/100 | Evaluated | ✅ | — | word placeholder |
| 3 | 2026-01-07 | Acme | Hays | Backend Engineer | 82/100 | Applied | ✅ | — | via agency |
| 4 | 2026-01-08 | Acme | — | Backend Engineer | 82/100 | Applied | ✅ | — | direct too |
`;
...
{
  const EMPTY_PDF = '| 42 | 2026-01-01 | Foo | Bar Engineer | 80/100 | Evaluated |  | [42](reports/042-foo-2026-01-01.md) | some note |';
  const EMPTY_NOTES = '| 43 | 2026-01-02 | Baz | Platform Engineer | 82/100 | Evaluated | ✅ | [43](reports/043-baz-2026-01-02.md) |  | Singapore';
