...
function validateEvaluationShape(text) {
  ...
  if (!summary) {
    ...
    if (!Number.isFinite(scoreValue) || scoreValue < 0 || scoreValue > 100) {
      issues.push('SCORE_SUMMARY score must be a number between 0 and 100');
    }
  }
...
function normalizedTrackerScore(value) {
  ...
  return /\/100$/i.test(clean) ? clean : `${clean}/100`;
}
...
ROLE: <role title>
SCORE: <global score as integer 0-100, e.g. 76>
...
console.log('\n' + '─'.repeat(66));
console.log(`  Score: ${score}/100  |  Archetype: ${archetype}  |  Legitimacy: ${legitimacy}`);
console.log('─'.repeat(66) + '\n');
